...
function initView(D, E) {
    ...
    if (N) { GroupEditTools.init(D, N) }
    var J = $("group-memberlist");
    if (J) { MembersList.init(D, E) }
    var K = $("select-favorite-button");
    var G = $("deselect-favorite-button");
    if (K || G) {
        GroupFavoriteSelector.init(null, E, D, function() { window.location.replace(window.location.href) });
        if (K) {
...
var startReportingModeObserver = function(C) {
    ...
    D.select(".RoomsWidget").each(function(E) { Event.observe(E, "click", this.bringToTop.bindAsEventListener(E), false) });
    D.select(".ProfileWidget").each(function(E) { Event.observe(E, "click", this.bringToTop.bindAsEventListener(E), false) });
    ...
    A.each(function(E) { Element.hide(E) });
    if (!reportingButtonsObserved) {
        ...
        $$(".es_dynamic_animator_sticker").each(function(E) {
            ...
            var F = Builder.node("img", { src: habboStaticFilePath + "/images/myhabbo/buttons/report_button.gif", "class": "animator-report-button" });
            E.appendChild(F);
            setTimeout(function() {
...
var stopReportingModeObserver = function(A) {
    Event.stop(A);
    var C = $("playground");
    C.select(".report-button").each(function(D) { Element.hide(D) });
    $$(".reporting-start").each(function(D) { Element.hide(D) });
    reportingZ = 8100;
    C.select(".stickie").each(function(D) { Event.stopObserving(D, "click", this.bringToTop, false) });
    C.select(".ProfileWidget").each(function(D) {
        Event.stopObserving(D, "click", this.bringToTop, false)
    });
    C.select(".RoomsWidget").each(function(D) { Event.stopObserving(D, "click", this.bringToTop, false) });
    C.select(".sticker").each(function(D) { Element.show(D) });
    C.select(".FriendsWidget").each(function(D) { Element.show(D) });
    C.select(".HighScoresWidget").each(function(D) { Element.show(D) });
    $$(".es_dynamic_animator_sticker").each(function(E) {
        var D = E.getElementsByTagName("img");
        if (D.length > 0) { Element.remove(D[0]) }
    });
    for (x in oldZ) { el = $(x); if (el) { el.style.zIndex = oldZ[x] } }
    oldZ = new Array();
...
function observeAnim() {
    ...
    if (D.length > 0) {
        ...
        Event.observe(D[0], "dblclick", function(A) {
            if (R < 100) {
                Event.stop(A);
                for (i = 0; i < DIL; i++) { new Effect.Move(DI[i], { x: parseFloat(Math.sin(R * x1 + i * x2 + x3) * x4 + x5), y: parseFloat(Math.cos(R * y1 + i * y2 + y3) * y4 + y5), mode: "absolute" }) }
                setTimeout(function() { B = setInterval(C, 10) }, 1000)
            }
        })
    }
    ...
    function C() {
        ...
        R++;
        if (R > 100) { clearInterval(B); for (i = 0; i < DIL; i++) { new Effect.Move(DI[i], { x: parseFloat(bckup[DI[i].id + "-l"]), y: parseFloat(bckup[DI[i].id + "-t"]), mode: "absolute" }) } }
    }
}
...
function letItSnow() {
    ...
    if (I == Q.getMonth() && M == Q.getDate() && Q.getHours() >= P) {
        ...
        for (i = 0; i < F; i++) {
            ...
            } else { J = '<div class="the_stickr sticker s_ss_snowflake1" style="left: ' + H[i] + "px; top: " + G[i] + 'px; z-index: 100" id="' + L + '">' }
            $("playground").insert(J);
            Element.hide($(L));
            new Effect.Appear($(L))
        }
        ...
        var C = 0;
        var K = 0;
        setTimeout(function() { K = setInterval(E, 80) }, 1000);
...
function openGroupActionDialog(J, H, F, D, E, C) {
    var A = "9001";
    if (E != null && E.widgetId != 0) { Overlay.show() } else {
        Overlay.move("9002");
        A = "9003"
    }
...
function openGroupJoinWithInviteDialog(A, D) {
    ...
    new Ajax.Request(habboReqPath + A, {
        method: "post",
        parameters: { groupId: D, inviteNeeded: true },
        onComplete: function(G, F) {
            Dialog.setDialogBody(C, G.responseText);
            if ($("group-action-ok")) {
                Event.observe($("group-action-ok"), "click", function(H) {
                    ...
                    new Ajax.Request(habboReqPath + A, {
                        method: "post",
                        parameters: { groupId: D, inviteNeeded: true, inviteCode: I },
...
function hideGroupActionDialog(A) { $("group-action-dialog").remove(); if (A != null && A.widgetId != null && A.widgetId != 0) { Overlay.hide() } else { Overlay.move("9000") } }
...
function showGroupSettingsConfirmation(D) {
    ...
    var E = Form.getInputs($("group-settings-form"), "radio", "group_type").find(function(J) { return J.checked }).value;
    ...
    if (G == "" || H == 0) { confirmAndUpdateGroupSettings(D) } else {
        new Ajax.Request(habboReqPath + "/groups/actions/check_group_url", {
            ...
            onComplete: function(N, M) {
                ...
                var L = Builder.node("a", { href: "#", className: "new-button" }, [Builder.node("b", L10N.get("myhabbo.groups.confirmation_ok")), Builder.node("i")]);
                var P = Builder.node("a", { href: "#", className: "new-button" }, [Builder.node("b", L10N.get("myhabbo.groups.confirmation_cancel")), Builder.node("i")]);
...
function confirmAndUpdateGroupSettings(C) {
    var D = Form.getInputs($("group-settings-form"), "radio", "group_type").find(function(F) { return F.checked }).value;
    ...
    if (D != A) {
        ...
        Dialog.showConfirmDialog(L10N.get("group.settings.group_type_change_warning." + E[parseInt(D)]), {
            ...
            cancelHandler: function() { $("group-settings-update-button").removeClassName("disabled") },
...
function openGroupSettings(C) {
    ...
    A.show();
    A.style.zIndex = "9001";
    new Ajax.Updater("dialog-group-settings-body", habboReqPath + "/groups/actions/group_settings", { parameters: { groupId: C }, evalScripts: true, method: "post" });
    Overlay.show();
    Dialog.moveDialogToCenter(A);
    Event.observe("group-settings-link-group", "click", function(D) { switchGroupSettingsTab(D, "group") });
...
function switchGroupSettingsTab(D, A) {
    ...
    if (!C.hasClassName("selected")) {
        $A(["group", "room"]).without(A).each(function(E) {
...
var MembersList = {
    init: function(A, C) {
        ...
        if (MembersList.pendingDiv) { Element.hide(MembersList.pendingDiv) }
        Element.hide(MembersList.memberButtonsDiv);
        if (MembersList.pendingButtonsDiv) { Element.hide(MembersList.pendingButtonsDiv) }
        MembersList.loadingMembers = false;
        MembersList.loadingPending = false;
        MembersList.loadedMembers = 0;
        MembersList.loadedPending = 0;
        Event.observe("group-memberlist-link-members", "click", function(D) { Event.stop(D); if (MembersList.selected != "members") { MembersList.switchToMembers(); if (new Date().getTime() - MembersList.loadedMembers > 10000) { MembersList.loadMembers(true, 1) } } });
            ...
            new Ajax.Updater("group-memberlist-members", habboReqPath + "/myhabbo/groups/memberlist", {
                ...
                onComplete: function(F, E) {
                    ...
                    Event.observe(MembersList.membersDiv, "click", function(H) {
                        ...
                        if (G && G.id && G.id.lastIndexOf("-") != -1) { var I = parseFloat(G.id.substring(G.id.lastIndexOf("-") + 1)); if (I > 0) { MembersList.loadAvatarInfo(I) } return }
                    })
                }
            })
        }
    ...
    processSearch: function(G) {
        ...
        if (D.id == "member-list-paging") { return }
        ...
        if (F == null) { F = 0 }
        var E = 1;
        if (D.id == "memberlist-search-first") { E = 1 } else {
    ...
    clickCheckbox: function() {
        if (MembersList.selected == "members") {
            ...
            $A(MembersList.membersDiv.getElementsByTagName("input")).each(function(G) {
                if (G.checked) {
                    if (G.id.substring(17, 18) == "a") { C = true } else { A = true }
            ...
            if (MembersList.selected == "pending") {
                var E = false;
                $A(MembersList.pendingDiv.getElementsByTagName("input")).each(function(G) { if (G.checked) { E = true; throw $break } });
                if (E) {
    ...
    confirm: function(C) {
        ...
        $A(A.getElementsByTagName("input")).each(function(F) { if (F.checked) { E.push(F.id.substring(F.id.lastIndexOf("-") + 1)) } });
        if (E.length > 0) {
            Overlay.move("9002");
                            ...
                            new Ajax.Request(habboReqPath + "/myhabbo/groups/batch/" + C, {
                                ...
                                onComplete: function(J, I) {
                                    if (J.responseText == "OK") {
                                        MembersList.closeConfirmationDialog();
                                        if (MembersList.selected == "pending") { MembersList.loadPending(true) } else {
                                            MembersList.loadMembers(true, 1)
                                        }
                                    } else {
...
function closeBadgeEditor(A) {
    Overlay.hide();
    if (A != null) { Event.stop(A) }
    Element.remove("badge-editor-dialog")
}

function loadWebStore(A) { ScriptLoader.load("myhabbo-store", { callback: A || null }) }
...
function getElementsInInvalidPositions() {
    var A = [];
    $("playground").select(".movable").each(function(C) { if (isNotWithinPlayground(C)) { A.push(C) } });
    return A
}
...
var Discussions = {
    ...
    initialize: function(D, A, C) {
        if (!Discussions.initialized) {
            ...
            if (Discussions.topicId) { Discussions.baseParams += "&topicId=" + Discussions.topicId }
            if ($("group-postlist-container")) { Event.observe($("group-postlist-container"), "click", Discussions.handleActions) }
    ...
    handleActions: function(I) {
        ...
        if (F.up("a.new-button")) { F = F.up("a.new-button") }
    ...
    showEditElements: function() {
        if ($("new-topic-entry-label")) { Element.show("new-topic-entry-label") }
    ...
    hideEditElements: function() {
        if ($("new-topic-entry-label")) { Element.hide("new-topic-entry-label") }
    ...
    showCaptchaPreview: function() { Discussions.swapCaptcha($("discussion-captcha"), $("discussion-captcha-preview")) },
    ...
    cancelPost: function() {
        ...
        if ($("new-post-preview")) { Element.hide("new-post-preview") }
        Pinger.stop()
    },
    cancelTopic: function() { location.href = Discussions.redirectLocation },
    createPost: function() {
        $("post-message").value = "";
        $("post-id").value = null;
        Discussions.showEditElements();
    ...
    createTopic: function() {
        ...
        new Ajax.Updater("group-topiclist-container", habboReqPath + "/discussions/actions/newtopic", {
            ...
            onComplete: function(D, C) {
                ...
                Discussions.showEditElements();
                if ($("new-topic-name")) { $("new-topic-name").focus() }
                Pinger.start()
            }
        })
    },
    previewPost: function() { Discussions.preview(null) },
    ...
    checkCaptcha: function() {
        var C;
        if ($("recaptcha_challenge")) { C = $("recaptcha_response_field") } else { C = $("captcha-code") }
        if (!C) { return true }
        var A = C.value;
        if (A.length == 0) {
    ...
    savePost: function() {
        if (!Discussions.checkCaptcha()) { return }
    ...
    save: function(F) {
        ...
        } else { if ($("captcha-code")) { H = "&captcha=" + $("captcha-code").value } }
                ...
                if (K && K.spam == "true") {
                    ...
                    if (K && K.forbidden == "true") { $("discussionbox").innerHTML = L.responseText; return } else {
                        if (K && K.captchaError == "true") {
                            ...
                            } else { Element.show("topic-form-save") }
                            Discussions.showErrorBubble("captcha-code-error", L10N.get("register.error.security_code"));
                            if ($("recaptcha_challenge")) { $("recaptcha_response_field").focus() } else { $("captcha-code").focus() }
                        } else {
                            if (E == "post") {
    ...
    saveTopicSettings: function(I) {
        ...
        var J = A.topic_type;
        for (var F = 0; F < J.length; F++) { if (J[F].checked) { D = J[F].value } }
        var K = A.topic_sticky;
        for (var F = 0; F < K.length; F++) { if (K[F].checked) { G = K[F].value } }
...
var Pinger = {
    ...
    onTimerEvent: function() {
            ...
            onSuccess: function(A, C) {
                if (C && C.privilegeLevel != 1) {
                    if (!Pinger.alreadyNotified) {
                        ...
                        if ($("post-form-save")) { $("post-form-save").addClassName("disabled-button") }
                        if ($("topic-form-save")) { $("topic-form-save").addClassName("disabled-button") }
                    }
        ...
        Pinger.invokeCount++;
        if (Pinger.invokeCount > 5) { Pinger.stop() }
    },
    stop: function() {
        if (Pinger.timer != null) {
            Pinger.timer.stop();
...
var WidgetRegistry = {
    ...
    add: function(A, D, C) {
        ...
        if (!WidgetRegistry._widgetsByType[D]) { WidgetRegistry._widgetsByType[D] = [] }
...
var FriendsWidget = Class.create({
    ...
    removeFromGroup: function(A) { Event.stop(A) },
    revokeRights: function(A) { Event.stop(A) },
    _parseAccountIdFromElementId: function(A) { return A.substring(A.lastIndexOf("-") + 1) },
    ...
    _processSearch: function(F) {
        ...
        var D = 1;
        if (C.id == "avatarlist-search-first") { D = 1 } else {
...
var MemberWidget = Class.create(FriendsWidget, {
    ...
    _addLinkObservers: function($super) {
        this.infoEl.select(".avatar-info-rights-leave").each(function(A) { A.onclick = this.leaveFromGroup.bindAsEventListener(this) }.bind(this));
        this.infoEl.select(".avatar-info-rights-remove").each(function(A) { A.onclick = this.removeFromGroup.bindAsEventListener(this) }.bind(this));
        this.infoEl.select(".avatar-info-rights-give").each(function(A) { A.onclick = this.giveRights.bindAsEventListener(this) }.bind(this));
        this.infoEl.select(".avatar-info-rights-revoke").each(function(A) { A.onclick = this.revokeRights.bindAsEventListener(this) }.bind(this))
    }
});
...
ScrollWatcher.prototype = {
    initialize: function(C, A, D) {
        ...
        new PeriodicalExecuter(function() { this.update(this) }.bind(this), 0.8)
    },
    update: function(A) {
...
GuestbookWidget.prototype = {
    initialize: function(F, E, D) {
        ...
        if ($("guestbook-open-dialog")) {
            ...
            Event.observe("guestbook-form-preview", "click", function(G) {
                Event.stop(G);
                if ("true" == $("guestbook-form-preview").disabled) { return }
                        ...
                        onComplete: function() {
                            Event.observe("guestbook-form-post", "click", function(H) {
                                ...
                                new Ajax.Updater($("guestbook-entry-container"), habboReqPath + "/myhabbo/guestbook/add", {
                                    ...
                                    insertion: "top",
                                    onComplete: function(J, I) {
                                        if (I && I.spam == "true") { return }
                                        if ($("guestbook-empty-notes")) {
                                            Element.hide("guestbook-empty-notes")
                                        }
                                        if (Prototype.Browser.IE) { $$("#guestbook-form-dialog .new-button").each(function(L) { Element.hide(L) }) }
                                        Element.hide("guestbook-form-dialog");
    ...
    handleActions: function(E) {
        ...
        if (D.className == "gbentry-report") {
            ...
            var A = 0;
            if (Prototype.Browser.IE) { A = $("guestbook-entry-container").scrollTop }
            ReportDialogManager.show("guestbook", C, D, { setWidth: false, setHeight: false, offsetTop: A, offsetLeft: -120 })
        }
    },
...
RatingObserver.prototype = {
    ...
    handleEvent: function(A) {
        var C;
        Event.stop(A);
        if (this.innerHtmlParamName != null) { C = this.commonAjaxParams.parameters + "&" + this.innerHtmlParamName + "=" + $(this.elementToObserve).innerHTML } else { C = this.commonAjaxParams.parameters }
        $(this.commonAjaxParams.elementToUpdate).innerHTML = "";
        new Ajax.Updater(this.commonAjaxParams.elementToUpdate, habboReqPath + this.urlToCall, { method: "get", parameters: C, evalScripts: false, insertion: "bottom", onComplete: this.commonAjaxParams.onCompleteFunction })
    }
};
...
RatingWidget.prototype = {
    ...
    refreshObservers: function() {
        ...
        for (var A = 0; A < C.length; ++A) { new RatingObserver(this.commonObserverParams, C[A], "/myhabbo/rating/rate", "givenRate") }
    }
};
...
GroupsWidget.prototype = {
    initialize: function(A, C) {
        ...
        if (typeof this.containerEl == "undefined") { return }
    ...
    showGroupDetails: function(C) {
        if (Event.element(C).nodeName.toLowerCase() == "a") { return }
        Event.stop(C);
        ...
        new Ajax.Request(habboReqPath + "/myhabbo/groups/groupinfo", {
            ...
            onComplete: function(E, D) {
                ...
                this.infoEl.select(".groups-info-deselect-favorite").each(function(F) { F.onclick = GroupFavoriteSelector.deselectFavorite.bindAsEventListener(this) }.bind(this))
            }.bind(this)
        })
    },
...
TagWidgetPartial.prototype = {
    initialize: function(A) {
        ...
        Event.observe("profile-tag-list", "click", function(C) { this.handleClickTag(C) }.bind(this));
    ...
    handleAddTag: function(D) {
        ...
        if (this.parentWidget.tagType == "group") { this.addGroupTag(C, this.parentWidget.groupId) } else { if (this.parentWidget.tagType == "avatar") { this.addAvatarTag(C, this.parentWidget.accountId) } }
        A.value = ""
    },
...
ProfileWidget.prototype = {
    ...
    initialize: function(C, A) {
        this.options = Object.extend({ messageText: "Add as friend?", headerText: "Are you sure?", loginText: "" }, arguments[2] || {});
        this.accountId = C;
        if (A) {
    ...
    handleAddFriend: function(A) {
        ...
        if (this.loggedInAccountId) {
            Event.stop(A);
            this.dialog = Dialog.showConfirmDialog(this.options.messageText, { okHandler: this.doSendFriendRequest.bind(this), headerText: this.options.headerText, buttonText: this.options.buttonText, cancelButtonText: this.options.cancelButtonText })
        } else {
            Event.stop(A);
...
HighscoreListWidget.prototype = {
    ...
    selectGameId: function() {
        ...
        $A(A.options).each(function(C) { if (C.value == this.gameId) { A.selectedIndex = C.index } }.bind(this))
    },
    handleScoresClick: function(E) {
        ...
        if (C.nodeName.toLowerCase() == "li" && C.id) {
            ...
            } else { if (C.id.indexOf("highscorelist-type") != -1) { A = C.id.split("-").last(); if (A == this.type) { return } } else { if (C.id.indexOf("highscorelist-page") != -1) { D = C.id.split("-").last() } } }
    ...
    setGameId: function(A) {
        if (A != "" && A != this.gameId) {
            ...
            new Ajax.Request(habboReqPath + "/myhabbo/highscorelist/setGameId", {
                method: "post",
                parameters: { widgetId: this.widgetId, gameId: A },
    ...
    _loadScores: function(C, E, D) {
        ...
        if (D != -1) { A = habboReqPath + "/myhabbo/highscorelist/page" } else { Element.wait(this.scoresEl) }
        new Ajax.Request(habboReqPath + A, {
            method: "post",
...
HallOfFameWidget.prototype = {
    ...
    _updateContent: function(A) {
        new Ajax.Updater(this.containerEl, habboReqPath + "/myhabbo/halloffame/ajax", {
            method: "post",
            parameters: { game: A, widgetId: this.widgetId },
            onComplete: function() {
                this._setupGameLinks()
            }.bind(this)
        })
...
var BadgesWidget = Class.create({
    ...
    _processSearch: function(F) {
        ...
        var D = null;
        if (C.id == "badge-list-search-first") { D = 1 } else { if (C.id == "badge-list-search-previous") { D = A - 1 } else { if (C.id == "badge-list-search-next") { D = A + 1 } else { if (C.id == "badge-list-search-last") { D = E } } } }
        if (null == D) { return }
